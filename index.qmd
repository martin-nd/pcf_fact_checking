...
metric_df = data.frame(
  ...
  `SDGE Mean` = c(
    mean(sdge_saidi$value),
    mean(sdge_saifi$value),
    mean(sdge_caidi$value)
  ),
  `CA Median Mean` = c(
    mean(median_saidi$value),
    mean(median_saifi$value),
    mean(median_caidi$value)
  ),
...
rates_viz_df = rates_viz_df %>%
  mutate(
    rank = ifelse(
      (!is.na(rank)) & (rank <= 3),
      sprintf('%s: Rank %s', data_year, rank),
      NA
    )
  )
...
ggplot(
  rates_viz_df,
  aes(x = data_year, y = cpkwh_residential, color = rate, group = rate)
) +
...
ggplot(
  sdge_av,
  aes(x = data_year, y = value, color = utility_name, group = utility_name)
) +
